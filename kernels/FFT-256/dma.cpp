...
void init_dma1() {
  ...
  if (dma1_control_fd < 0) {
    fprintf(stderr, "[ERROR] Can't open /dev/mem. Exiting ...\n");
    exit(1);
  }
  ...
  if (dma1_control_base_addr == MAP_FAILED) {
    fprintf(stderr, "[ERROR] Can't obtain memory map to DMA1 control slave. Exiting ...\n");
    exit(1);
  }
...
void init_dma2() {
  ...
  if (dma2_control_base_addr == MAP_FAILED) {
    fprintf(stderr, "[ERROR] Can't obtain memory map to DMA2 control slave. Exiting ...\n");
    exit(1);
  }
...
void init_udmabuf1() {
  ...
  if (fd_udmabuf1 < 0) {
    fprintf(stderr, "[ERROR] Can't open /dev/udmabuf0. Exiting ...\n");
    exit(1);
  }

  // printf("[ INFO] Successfully opened /dev/udmabuf0 ...\n");

  udmabuf1_base_addr = (unsigned int *)mmap(NULL, 8192, PROT_READ | PROT_WRITE, MAP_SHARED, fd_udmabuf1, 0);

  if (udmabuf1_base_addr == MAP_FAILED) {
    fprintf(stderr, "[ERROR] Can't obtain memory map to udmabuf0 buffer. Exiting ...\n");
    exit(1);
  }
  ...
  if (fd_udmabuf_addr < 0) {
    fprintf(stderr, "[ERROR] Can't open /sys/class/udmabuf/udmabuf0/phys_addr. "
                    "Exiting ...\n");
    exit(1);
  }
...
void init_udmabuf2() {
  ...
  if (fd_udmabuf2 < 0) {
    fprintf(stderr, "[ERROR] Can't open /dev/udmabuf1. Exiting ...\n");
    exit(1);
  }

  // printf("[ INFO] Successfully opened /dev/udmabuf1 ...\n");

  udmabuf2_base_addr = (unsigned int *)mmap(NULL, 8192, PROT_READ | PROT_WRITE, MAP_SHARED, fd_udmabuf2, 0);

  if (udmabuf2_base_addr == MAP_FAILED) {
    fprintf(stderr, "[ERROR] Can't obtain memory map to udmabuf1 buffer. Exiting ...\n");
    exit(1);
  }
  ...
  if (fd_udmabuf_addr < 0) {
    fprintf(stderr, "[ERROR] Can't open /sys/class/udmabuf/udmabuf1/phys_addr. "
                    "Exiting ...\n");
    exit(1);
  }
...
void close_dma1() {
  ...
  // ...\n"); printf("[ INFO] Closing file descriptor to DMA control
  // slave...\n");
}
