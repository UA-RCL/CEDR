...
// TODO: IL is broken right now as it relies on the Baseline CEDR task structures. Adapt it to API-based version

extern "C" {

// Get resource by doing max of probabilities of the softmax layer at the end of the DNN
int nn_max_ml_sched(int dim_0, NN_NUM_TYPE array[dim_0]) {
...
void print_array_1d(int dim_0, NN_NUM_TYPE array[dim_0]) {
  double write_var;
...
int getPrediction(ConfigManager &cedr_config, NN_DATA_ML_SCHED *nn_data, task_nodes *task, worker_thread *wt, int model) {
  ...
  for (feature_index = 0; feature_index < cedr_config.getResourceArray()[resource_type::cpu] + cedr_config.getResourceArray()[resource_type::fft] +
                                              cedr_config.getResourceArray()[resource_type::mmult];
       feature_index++) {
