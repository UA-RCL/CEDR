...
if __name__ == "__main__":
    ...
    WORKLOAD = 'High'
    TRIALS = 5
    schedlist = {'SIMPLE':1, 'MET':2, 'EFT':3, 'ETF':4, 'HEFT_RT':5}
    schedmarkerlist = {'SIMPLE':'o', 'MET':'v', 'EFT':'^', 'ETF':'s', 'HEFT_RT':'d'}
    schednamelist = ['RR', 'MET', 'EFT', 'ETF', 'HEFT$_\mathrm{RT}$']
    #colorlist = {' Core 1':'green', ' Core 2': 'blue', ' Core 3': 'red', ' FFT 1':'black', ' MMULT 1': 'orange'}
    ...
    for c in range(CPUS):
        for f in range(FFTS+1):
            for m in range(MMULTS+1):
                configlist.append('C'+str(c+1)+'+F'+str(f)+'+M'+str(m))
                configlist_tick.append('C'+str(c+1)+'\nF'+str(f)+'\nM'+str(m))
                #configlist_tick.append('        C'+str(c+1)+'F'+str(f)+'M'+str(m))
                #configlist_tick.append('C'+str(c+1)+'F'+str(f)+'M'+str(m))
    print(configlist)
    ...
    ax.set_yticklabels(configlist_tick, fontsize=20)
