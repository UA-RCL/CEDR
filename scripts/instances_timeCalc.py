...
def create_gantt_chart(file, name):
    """
...
def generate_argparser():
    ...
    parser.add_argument("-e", "--experiment_number", help="Experiment number. Experiment is * by default", default="*")
    parser.add_argument("-g", "--garnt", help="Creates a garnt chart of indicated experiment number (-e)", nargs='?', const='')
    return parser
...
def main():
    ...
    proc_schedules = {}
    expNum = args.experiment_number
    files = glob.glob(args.path_to_experiments + "/experiment" + expNum + "/timing_trace.log")
    if(files == []):
    ...
    for file in files:
        ...
        try:
            if(app_starts != {}):
                exec_times += [np.mean([(app_ends[i] - app_starts[i])/1e6 for i in range(len(app_starts))])]
        except:
            print("Error when caculating execution times at line 117")
            print("Did all instances actually finish to execture before killing CEDR?")
    print("Averages of all instances of all sub_dags (ms): ", np.mean(exec_times))

    if((args.garnt != None) & (args.experiment_number!= None)):
        create_gantt_chart(files[0], "experiment" + args.experiment_number + "_Garnt")
    pass
