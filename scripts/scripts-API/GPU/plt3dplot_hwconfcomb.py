...
if __name__ == "__main__":
    ...
    WORKLOAD = 'High'
    TRIALS = 3
    schedlist = {'SIMPLE':1, 'MET':2, 'EFT':3, 'ETF':4,}
    schedmarkerlist = {'SIMPLE':'o', 'MET':'o', 'EFT':'o', 'ETF':'o'}
    schednamelist = ['RR', 'MET', 'EFT', 'ETF']
            ...
            for m in range(MMULTS+1):
              for z in range (ZIPS+1):
                for g in range (GPUS+1):
                  resourceCount = ((c+1) + f + m + z + g)
                  if resourceCount not in configlist:
                    configlist.append(resourceCount)#'C'+str(c+1)+'+F'+str(f)+'+M'+str(m)+'+Z'+str(z))
                    configlist_tick.append(resourceCount)#'C'+str(c+1)+'\nF'+str(f)+'\nM'+str(m)+'\nZ'+str(z))
                  #configlist_tick.append('        C'+str(c+1)+'F'+str(f)+'M'+str(m)+'Z'+str(z))
                  #configlist_tick.append('C'+str(c+1)+'F'+str(f)+'M'+str(m)+'+Z'+str(z))
    print(configlist)
    metrics = ['Avg. cumulative execution time / app. (ns)', 'Avg. run time / app.(ns)', 'Avg. Scheduling overhead / app.(ns)']
