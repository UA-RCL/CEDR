...
def resource_util(FILENAME):
    ...
    with open(FILENAME, newline='') as fp:
        ...
        stop_dict = {}
        exectime_dict = {}
        corelist = [' cpu1', ' cpu2', ' cpu3', ' cpu4', ' cpu5', ' cpu6', ' cpu7', ' fft1',' fft2',' fft3',' fft4',' fft5',' fft6', ' fft7', ' fft8', ' mmult1', ' mmult2', ' zip1', ' zip2', ' gpu1']  # Edit here
                        ...
                        for c in range (CPUS):
                            for sched in SCHEDS:
                                for i in range(INJ_COUNT):
                                    row_to_write = []
                                    resPoolComb = 'C'+str(c+1)+'+F'+str(f)+'+M'+str(m)+'+Z'+str(z)+'+G'+str(g) # Edit here
                                    ...
                                    for t in range(TRIALS):
                                        file_to_parse = BASEDIR+'/'+WORKLOAD+'/trial_'+str(t+1)+'/c'+str(c+1)+'_f'+str(f)+'_m'+str(m)+'_z'+str(z)+'_g'+str(g)+'_sched-'+sched+'-p0,'+str(WF_PERIODS[i])+','+str(WF_PERIODS[i])+','+str(PD_PERIODS[i])+'/timing_trace.log' # Edit here
                                        sched_file_to_parse = BASEDIR+'/'+WORKLOAD+'/trial_'+str(t+1)+'/c'+str(c+1)+'_f'+str(f)+'_m'+str(m)+'_z'+str(z)+'_g'+str(g)+'_sched-'+sched+'-p0,'+str(WF_PERIODS[i])+','+str(WF_PERIODS[i])+','+str(PD_PERIODS[i])+'/schedule_trace.log' # Edit here
                                        apprun_file_to_parse= BASEDIR+'/'+WORKLOAD+'/trial_'+str(t+1)+'/c'+str(c+1)+'_f'+str(f)+'_m'+str(m)+'_z'+str(z)+'_g'+str(g)+'_sched-'+sched+'-p0,'+str(WF_PERIODS[i])+','+str(WF_PERIODS[i])+','+str(PD_PERIODS[i])+'/appruntime_trace.log'
                                        print(file_to_parse)
