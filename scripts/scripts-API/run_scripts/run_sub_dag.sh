...
APPS=("apps/track_fft.so,apps/wifi-tx-aarch64.so,apps/wifi-tx-aarch64.so,apps/pulse_doppler-aarch64.so") # "apps/correlator.json,apps/temporal_mitigation.json")
INSTS=("1,25,25,20")
              ...
              for (( cpu=1; cpu<=$CPUS; cpu++ )); do
                for sched in "${SCHEDS[@]}"; do
                  ...
                  do # ! test -f $FILE ; do
                    ...
                    ID=`echo $cedr | tr ' ' '\n' | head -n 1` #2 | tail -n 1` # Un-comment this for GPU
