...
cufftDoubleComplex *d_fft256, *d_fft512, *d_fft1024, *d_fft2048;
...
//    cudaMalloc((void**) &d_fft1024, sizeof(cufftDoubleComplex)*1024);
    cufftPlan1d(&plan_fft1024, 1024, CUFFT_Z2Z, 1);
...
extern "C" void fft_cuda(double* input, double* output, int64_t fft_size, bool isForwardTransform, cufftHandle plan, cufftDoubleComplex *d_data) {
    //printf("===========================================================\n");
    //printf("=============== Executing %ld-Pt %s on GPU ================\n", fft_size, isForwardTransform ? "FFT" : "IFFT");
    //printf("===========================================================\n");

    //double *tmp_d_data = (double*) d_data;
    cudaMemcpy(d_data, input, 2 * fft_size * sizeof(input[0]), cudaMemcpyHostToDevice);

    cufftExecZ2Z(plan, d_data, d_data, isForwardTransform ? CUFFT_FORWARD : CUFFT_INVERSE);
    cudaDeviceSynchronize();
    
    cudaMemcpy(output, d_data, 2 * fft_size * sizeof(output[0]), cudaMemcpyDeviceToHost);
}

extern "C" void DASH_FFT_gpu(double** input, double** output, size_t* size, bool* isForwardTransform){
  ...
  cudaMalloc((void**) &d_fft, sizeof(cufftDoubleComplex)*(*size));
  if(*size == 256){
...
//  	cufftDoubleComplex *d_fft_custom;
//  	cufftHandle plan_fft_custom;
  	cufftPlan1d(&plan_fft_custom, (*size), CUFFT_Z2Z, 1);
