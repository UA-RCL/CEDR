...
/* Input argument
 * mode =  TX or RX
 * for TX pass the argument TXMODE
 * for RX pass the argument RXMODE
*/ 
int asu_radio_init(int mode)
{
...
// END LIBRADIO
    ...
    // BEGIN TX TX TX
/* input arguments
 * input_buffer_add = Address to the buffer which needs to be transmitted
 * insize = size of input buffer in bytes
 * output_buffer_add = Address to which the received IQ values should be stored
 * radio_test_time = Time for which radio should be enabled */

int asu_tx_rx( unsigned int input_buffer_add, int insize, unsigned int output_buffer_add,
		int radio_test_time)
{
    int 	 ret;
    int          fd_rad;
    ...
    if (radio_test_time) {
        ...
        if (input_buffer_add != 0) {
            ...
            RadConfig.cfg4g.TxPlayBufUserVirtAddr = input_buffer_add;
            RadConfig.cfg4g.TxPlayBufSize = insize;
