...
void init_fft1() {
    ...
    fft1_control_base_addr = (unsigned int*) mmap(0, getpagesize(), PROT_READ|PROT_WRITE, MAP_SHARED, fft1_control_fd, FFT1_CONTROL_BASE_ADDR);
...
void init_fft2() {
	...
	// Obtain virtual address to DMA control slave through mmap
    fft2_control_base_addr = (unsigned int*) mmap(0, getpagesize(), PROT_READ|PROT_WRITE, MAP_SHARED, fft2_control_fd, FFT2_CONTROL_BASE_ADDR);
...
// Function to initialize memory maps to FFT 
//###################################################################################
void config_ifft(unsigned int *base) {
...
// Function to initialize memory maps to FFT 
//###################################################################################
void config_fft(unsigned int *base) {
