...
void* wifi_rx() {
    ...
    while(frame_count < NUM_FRAMES) {
       ...
       for(j=0; j<SYM_NUM; j++) {
           ...
           assert(pthread_create(&thread_payload, &attr_thread_payload, payloadExt, (void *)thread_param_payload) == 0);
           ...
           assert(pthread_create(&thread_pilotrm, &attr_thread_pilotrm, pilotRemove, (void *)thread_param_pilotrm) == 0);
           ...
           #else
           struct args_qpsk_demod *thread_param_qpsk_demod = (struct args_qpsk_demod *)malloc(sizeof(struct args_qpsk_demod));
           thread_param_qpsk_demod->n = INPUT_LEN;
           thread_param_qpsk_demod->ibuf = pilot_rm;
           thread_param_qpsk_demod->out = outbit;
           assert(pthread_create(&thread_qpsk_demod, &attr_thread_qpsk_demod, DeMOD_QPSK, (void *)thread_param_qpsk_demod) == 0);
           ...
           #else
           struct args_deinterleaver *thread_param_deinterleaver = (struct args_deinterleaver *)malloc(sizeof(struct args_deinterleaver));
           ...
           assert(pthread_create(&thread_deinterleaver, &attr_thread_deinterleaver, deinterleaver, (void *)thread_param_deinterleaver) == 0);
           ...
           assert(pthread_create(&thread_decoder, &attr_thread_decoder, viterbi_decoding, (void *)thread_param_decoder) == 0);
           ...
           #else
           struct args_descrambler *thread_param_descrambler = (struct args_descrambler *)malloc(sizeof(struct args_descrambler));
           ...
           assert(pthread_create(&thread_descrambler, &attr_thread_descrambler, descrambler, (void *)thread_param_descrambler) == 0);
