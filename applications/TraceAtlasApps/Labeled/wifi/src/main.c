...
int main() {
    ...
    pthread_t thread_wifi_tx, thread_wifi_rx, thread_pulse_doppler, thread_range_detection;
    ...
    int core_id = 4;
        ...
        int s1, s2, s3, s4;
        pthread_attr_t attr_thread_wifi_tx, attr_thread_wifi_rx, attr_thread_pulse_doppler, attr_thread_range_detection;
        ...
        pthread_attr_init(&attr_thread_pulse_doppler);
        CPU_ZERO(&cpuset3);
        ...
        s3 = pthread_attr_setaffinity_np(&attr_thread_pulse_doppler, sizeof(cpuset3), &cpuset3);
        ...
        if (sched_setaffinity(0, sizeof(cpu_set_t), &cpuset) != 0) {
            printf("[ERROR] Error in setting CPU affinity. Exiting ...\n");
            exit(0);
        }
...
//#define WIFI_TX
//#define WIFI_RX
//#define PULSE_DOPPLER
//#define RANGE_DETECTION
    ...
    #ifdef PULSE_DOPPLER 
	// Create thread for Pulse Doppler
        // Check status of thread create
        #ifdef ANDROID
        if (pthread_create(&thread_pulse_doppler, NULL, pulse_doppler, NULL) != 0) {
            printf("[ERROR] Pulse Doppler Thread Creation FAILED ...\n");
            exit(-1);
        } else {
            printf("[ INFO] Pulse Doppler Thread successfully created ...\n");
        }	
        #else
	    #ifdef PAPI
                pulse_doppler();
	    #else
                #ifdef FORCE_CPU_AFFINITY
                if (pthread_create(&thread_pulse_doppler, &attr_thread_pulse_doppler, pulse_doppler, NULL) != 0) {
                #else
                if (pthread_create(&thread_pulse_doppler, NULL, pulse_doppler, NULL) != 0) {
                #endif
                    printf("[ERROR] Pulse Doppler Thread Creation FAILED ...\n");
    ...
    //#################################################################
    //## Join threads of each of the applications
    //#################################################################
        ...
        #ifdef PULSE_DOPPLER
            // Join created threads
            if (pthread_join(thread_pulse_doppler, NULL) != 0) {
                    printf("[ERROR] Pulse Doppler Thread Join FAILED ...\n");
                    exit(-1);
                }
        #endif
    #endif
