...
/* __attribute__((always_inline)) */ void alternateinverse( float m[16], float invOut[16]);
#pragma SDS data data_mover(m:AXIDMA_SIMPLE,invOut:AXIDMA_SIMPLE)
...
/* __attribute__((always_inline)) */ void mmultiply(float A[N*N],float B[N*N],float C[N*N]);
#pragma SDS data access_pattern(S:SEQUENTIAL ,Si:SEQUENTIAL,Shermitian:SEQUENTIAL , Shermitianimag:SEQUENTIAL)
/* __attribute__((always_inline)) */ void hermitian(float S[N*M] ,float Si[N*M],float Shermitian[N*M],float Shermitianimag[N*M]);
float divide(float,float);

/* __attribute__((always_inline)) */ void mmadd(float A[N * N], float B[N * N], float C[N * N]);
