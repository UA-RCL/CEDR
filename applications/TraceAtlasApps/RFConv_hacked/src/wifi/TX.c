...
void readConfig() {
	...
	char *file0 = "Dash-RadioCorpus/QPR8_RFConvSys/tx.cfg";
...
void *wifi_tx() {
	...
	char *file2 = "Dash-RadioCorpus/QPR8_RFConvSys/txdata_remod.txt";
	char *path2 = (char *)malloc(FILEPATH_SIZE * sizeof(char));
	strcat(path2, getenv("DASH_DATA"));
	strcat(path2, file2);
	printf("Opening: %s", path2);
	FILE *txdata_file = fopen(path2, "w");
	free(path2);
	...
	while (frame_count < NUM_FRAMES) {
		...
		for (i = 0; i < SYM_NUM; i++) {
			...
			struct args_scrambler *thread_param_scrambler =
			    (struct args_scrambler *)malloc(sizeof(struct args_scrambler));
			thread_param_scrambler->inlen = USR_DAT_LEN;
			thread_param_scrambler->ibuf = &inbit[i * USR_DAT_LEN];
			thread_param_scrambler->obuf = scram;
			assert(pthread_create(&thread_scrambler, &attr_thread_scrambler, scrambler,
			                      (void *)thread_param_scrambler) == 0);
...
//###############################################################
			...
			assert(pthread_create(&thread_encoder, &attr_thread_encoder, viterbi_encoding,
			                      (void *)thread_param_encoder) == 0);
			...
			// + (double)start1.tv_nsec); printf("[INFO] TX-Puncturing execution time (ns): %f\n", exec_time); #endif
...
//###############################################################
			...
			struct args_interleaver *thread_param_interleaver =
			    (struct args_interleaver *)malloc(sizeof(struct args_interleaver));
			...
			assert(pthread_create(&thread_interleaver, &attr_thread_interleaver, interleaver,
			                      (void *)thread_param_interleaver) == 0);
...
#else
			/* Parse the structure that will be sent as arguments for the ifft_hs fx */
			struct args_cyclic_prefix *thread_param_crc =
			    (struct args_cyclic_prefix *)malloc(sizeof(struct args_cyclic_prefix));
