...
void set_viterbiDecoder(int dId) {
	...
	for (i = 0; i < SM_STEP; i++) {
		for (j = 0; j < SM_STATE; j++) {
			...
			if (i == 0)
				vd->smu[i][nState].prev[count] = &(vd->smu[SM_STEP - 1][j]);
			else
				vd->smu[i][nState].prev[count] = &(vd->smu[i - 1][j]);
...
void *viterbi_decoding(void *input) {
	...
	for (i = 0; i < SM_STEP; i++) {
		column = i;
		if (column == 0)
			viterbi_initSmu(dId, column, 1);
		else
			viterbi_initSmu(dId, column, 0);
