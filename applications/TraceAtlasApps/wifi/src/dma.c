...
void init_dma1() {
    ...
    dma1_control_base_addr = (unsigned int*) mmap(0, getpagesize(), PROT_READ|PROT_WRITE, MAP_SHARED, dma1_control_fd, DMA1_CONTROL_BASE_ADDR);
...
void init_dma2() {
	...
	// Obtain virtual address to DMA control slave through mmap
    dma2_control_base_addr = (unsigned int*) mmap(0, getpagesize(), PROT_READ|PROT_WRITE, MAP_SHARED, dma2_control_fd, DMA2_CONTROL_BASE_ADDR);
...
//###################################################################################
void setup_tx (unsigned int *base, unsigned int udmabuf_phys_addr) {

    dma_write_reg(base, DMA_OFFSET_MM2S_SRCLWR,  udmabuf_phys_addr);
    dma_write_reg(base, DMA_OFFSET_MM2S_CONTROL, 0x3);
    dma_write_reg(base, DMA_OFFSET_MM2S_LENGTH,  (DIM * 4 * 2));
...
//###################################################################################
void setup_rx (unsigned int *base, unsigned int udmabuf_phys_addr) {

    dma_write_reg(base, DMA_OFFSET_S2MM_SRCLWR,  (udmabuf_phys_addr + (DIM * 4 * 2)));
    dma_write_reg(base, DMA_OFFSET_S2MM_CONTROL, 0x3);
    dma_write_reg(base, DMA_OFFSET_S2MM_LENGTH,  (DIM * 4 * 2));
