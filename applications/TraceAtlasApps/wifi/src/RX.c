...
int main() {
    ...
    out_fd = (comp_t*) calloc(FFT_N, sizeof(comp_t));
    pilot_rm = (comp_t*) calloc(INPUT_LEN, sizeof(comp_t));
    pilotdata_rx = (float*) calloc(PILOT_LEN, sizeof(float));
    out_fft_c2f = (float*) calloc(INPUT_LEN * 2, sizeof(float));
   ...
   //frame_count++;
    ...
    printf("WiFi RX chain complete (took %f seconds)\n", exec_time);
    return 0;
}
