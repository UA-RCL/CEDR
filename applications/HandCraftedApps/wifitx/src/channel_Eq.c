 ...
 * Return Type  : void
 */
void channel_Eq(const creal_T tpilot[16], const creal_T rpilot[16], const
                creal_T fftout[64], const creal_T F[4096], creal_T eqData[64], int mode)
{
  creal_T b[256];
  creal_T dcv0[16];
  creal_T dcv1[256];
...
if(mode == 0) {
  ...
  for (i0 = 0; i0 < 16; i0++) {
    ...
    dcv0[i0].im = 0.0;
    for (i1 = 0; i1 < 16; i1++) {
      dcv0[i0].re += b[i0 + (i1 << 4)].re * rpilot[i1].re - b[i0 + (i1 << 4)].im
        * rpilot[i1].im;
      dcv0[i0].im += b[i0 + (i1 << 4)].re * rpilot[i1].im + b[i0 + (i1 << 4)].im
        * rpilot[i1].re;
    }
  }
